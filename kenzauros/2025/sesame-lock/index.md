...
- [SESAMEフェイス Pro](https://jp.candyhouse.co/products/sesame-face-pro) 5,980円 
または [SESAMEフェイス](https://jp.candyhouse.co/products/sesame-face) 5,480円 【ほぼ必須】
...
セサミでは公式に Web API (Sesame API) が提供されており、ロックの状態や利用履歴、解錠などをリモートで行えることも特長です。製品自体が安価なのにもかかわらず、APIまで無償で提供されていて、とても良心的な会社だという印象です。

- [Web API](https://doc.candyhouse.co/ja/SesameAPI)
...
実際に SESAME 5 Pro を購入して合わせてみたところ、追加のパーツも高さ調整も不要で、そのまま取り付けられました。下図は従来の SwitchBot ロックと SESAME 5 Pro の取り付けイメージです。

![](images/smart-lock-before-after.jpg "SwitchBot ロックと SESAME 5 Pro の取り付けイメージ")

取り付けた見た目でいうと、以前の SwitchBot ロックより少し高級感は劣りますが、かなりコンパクトになったので、すっきりした印象です。また SwitchBot のロック Pro は本体に押しボタン（クイックキー）がついており、それで解錠できたのですが、 SESAME 5 Pro にはそれがありません。ボタンでの解錠になれていたため、別途「リモート」を取り付けています。

高さ方向だけなら標準のねじで調整可能です。もしサムターン部分が合わない場合も、[特殊アダプター](https://jp.candyhouse.co/products/adapter_normal)が売られているほか、メーカーにお願いすればわずか600円でカスタムのアダプターを作ってもらえるそうです。これはすごいサービスですね。
...
ロックの設定や指紋認証・顔認証の登録などは SwitchBot や Qrio と比べても複雑なステップがなく、シンプルで高速な設定が可能だと思います。
...
![](images/app-device-list.png "セサミアプリのデバイス一覧")

なお、Hub3 を設置していても、デバイスの設定変更や電池残量の確認・アップデートなどは Bluetooth の範囲内でないとできません。特に電池切れが致命的になるデバイスなので、正しい電池残量がリモートで確認できるようになることを期待しています。

また、アプリに複数のアカウントを登録しておいて切り替えることができません。私は自宅でもセサミを利用しているので、個人アカウントと会社アカウントを切り替えたいのですが、いちいちログアウトしてログインし直す必要があり、面倒です。なお、ログアウトはちょっとわかりづらいのですが、アプリの「自分」タブの画面を上にスワイプすると「ログアウト」ボタンが表れます。
...
ただし、Hub3 を設置していない場合、履歴はスマホアプリを開いたときにしか取得・アップロードされません。定期的に履歴をアップロードしたい場合は、Hub3 を設置することを強くお勧めします。

ちなみにこの Hub3 、SwitchBot 社にハブと比べると驚くほど小さく、届いたときは何が入っている箱かわかりませんでした。 USB-C で給電するタイプで、AC アダプターや USB-C ケーブルは付属していないため、別途用意する必要があります。小さくていいのですが、逆にACアダプターやケーブルのほうがかさばってしまい、設置場所に悩みました。そんな悩みを見透かしてか（？）、[Hub3 コネクタ](https://jp.candyhouse.co/products/hub3_connector)という様々な形状のUSBアダプターも売られていたります。
...
まずロック（SESAME 5 Pro）自体の動作は、解錠・施錠ともに高速で、音もさほど大きくなく、正確に計測はしていませんが、 SwitchBot ロックと同程度の印象です。この点に関して、不満やストレスは今のところありません。

あと施錠・解錠は SwitchBot ロックのころにあった、引っ掛かりのような動きのときがなく、よりスムースな動作に感じます。 SwitchBot や Qrio よりサムターン部分の遊びを大きくして、逆に引っ掛かりを減らした設計に見えます。
...
ただ、人によっては、顔認証が始まるまでに時間がかかったり、顔の距離を何度か調整しないと通りづらい場合があるようです。顔を登録しなおしたり、複数登録すると改善される場合もあります。私も眼鏡あるなしで、通らなかったので、両方の顔を登録しています。

### 施錠や解錠の履歴について

当初の目的である施錠・解錠の履歴については、うまく SESAME Biz で確認でき、CSVでのダウンロードやAPIでの取得もできています。このあたりについてはまた別の記事で紹介します。
...
SwitchBot ロックの初期の製品や Qrio Lock もそうでしたが、 **CR123A** というリチウム電池が主に使われています。これはハイパワー・高容量であったり、温度特性や放電特性がよかったりという特長があるので、スマートロックによく用いられるのですが、入手性やコストの面ではかなり不利です。

SwitchBot ロックの場合は、通常の単3電池に対応させたり、専用のリチウムイオン充電池を発売したりして、電池問題の改善を図っていました。

セサミの場合は、**自社ブランドの CR123A を自社で安価に販売する**、というアプローチをとっています。通常 CR123A の小売り価格は 300 円以上、ネットでまとめ買いしても 200 円は下らないのですが、 CANDY HOUSE の公式通販サイトでは 1本 163 円という安価で販売しています。電池の容量も記載されているので、品質面でも安心感があります。

- [CR123A (4本セット) – SwitchBot (スイッチボット)](https://www.switchbot.jp/products/cr123a-4p?variant=42342873661615) 1,380円（1本あたり**345円**）
- [CR123A (1本) - CANDY HOUSE](https://jp.candyhouse.co/products/new_%E5%88%9D%E4%BB%A3%E3%82%BB%E3%82%B5%E3%83%9F-%E3%82%BB%E3%82%B5%E3%83%9Fmini-3-4-%E3%82%B5%E3%82%A4%E3%82%AF%E3%83%AB%E5%85%B1%E9%80%9A-%E9%AB%98%E5%AE%B9%E9%87%8F1800mah%E3%81%AEcr123a-%E9%9B%BB%E6%B1%A0-%E3%83%90%E3%83%83%E3%83%86%E3%83%AA%E3%83%BC) **163円**

エコの観点からどちらがよいかはわかりませんが、「スマートロックを広く使ってもらいたい」という CANDY HOUSE の思いは感じました。ちなみに SwitchBot も指紋認証パッドは CR123A でしか動作しません。
...
なお、アプリで「レーダーの探知距離」という設定項目があり、この距離を短くすることで、ある程度不要な起動を抑えることができます。現在この探知距離を 40cm ぐらいに設定して、運用してみています。ただこの場合、認証のために近づいても起動しないことがあり、プチストレスにはなります。ちょうどいい距離を見つけるまで仕方ないのかもしれません。ちなみにこのドアは出入りも多く、電池切れはかなり困るので、6本すべての電池を入れることにしました。
...
上述のように、電池の持ちやアプリの使い勝手など、いくつか改善してほしい点はありますが、総合的に見て、小規模オフィスの入退出管理用途としては、コストパフォーマンスに優れたソリューションといえます。

別の記事で SESAME APIを使って、施錠・解錠履歴を Google スプレッドシートに記録する方法を紹介する予定ですので、興味のある方はそちらもぜひご覧ください。
