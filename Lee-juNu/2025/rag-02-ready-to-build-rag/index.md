...
![PDF をテキストに変換した結果の一例](images/pp02.png)
...
![オーバーラップ例文](images/pp04.jpg)

例えば上記の例文では、Chunk1の末尾が「Our」で終わっており、文が途中で区切られていることが確認できます。そのためChunk2では、同じ文の文頭「dicts bounding boxes...」から始まっており、チャンク間で文単位の重複が行われていることがわかります。

したがって、オーバーラップを用いたチャンク分割は、長文を処理する際に意味の断絶を避け、モデルがより正確に文脈を理解・保持できるようにするために重要な手法といえます。
...
![chunk をベクトル化](images/pp05.png)


#### ベクトル化とは？

LLM は、テキストをそのまま理解することはできません。代わりに、入力されたテキストを数値ベクトル（埋め込みベクトル）に変換して処理します。

たとえば、単語を一次元ベクトルで任意の値で表現してみます。

塩味：0.1
醤油味：0.5
味噌味：0.8

ここで味を追加して「辛味噌味」を追加するのであれば0.8周りである 0.85ぐらいに配置されます。
次は単語をもう一つ追加して2次元ベクトルを作ります。
...
このように、単語ひとつひとつを数値で表現することで、意味の違いや類似性を機械が扱えるようになります。そして単語が複数組み合わさると、文や文書になります。

たとえば「辛味噌ラーメンを食べたい」という文は、単語ベクトルの組み合わせにより、全体として1つのベクトルに変換されます。例えば OpenAi の embedding model の場合は1,536次元の多次元ベクトルとして表現されます。
