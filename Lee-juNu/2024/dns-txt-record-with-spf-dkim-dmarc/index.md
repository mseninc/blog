...
| 名称                                 | 種類 | 本文                                                                                           | TTL  |
| ------------------------------------ | ---- | ---------------------------------------------------------------------------------------------- | ---- |
| selector-name._domainkey.example.com     | TXT  | v=DKIM1; p=76E629F05F70 9EF665853333 EEC3F5ADE69A 2362BECE4065 8267AB2FC3CB 6CBE             | 6000 |

#### 名称 

多くの DNS TXT レコードとは異なり、DKIM レコードはドメイン名だけではなく、特殊な名前で登録されます。DKIM レコードの名前は子のフォーマットに従います。
` [セレクタ名]._domainkey.[ドメイン名] `
セレクタ名は、ドメインが使用しているメールサービスプロバイダが発行する特殊な値です。メールサーバーが DNS で必要な DKIM 検索を実行できるように、DKIM ヘッダーに含まれています。ドメイン名は、メールドメイン名です。_domainkey.は、すべての DKIM レコード名に含めます。
