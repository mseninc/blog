...
**Red Hat Enterprise Linux 10（RHEL 10）** を正式にリリースしました。

この記事では、RHEL 10のサポート期間、主な変更点、そして実際のインストール手順について解説します。
...
- AWS / GCP / Azureとの統合強化  
- Webコンソールの改善（Stratis FS管理、HAクラスタ統合など）


## インストール手順とスクリーンショット

実際にインストールを行いましたので参考まで記載します。
...
直後のカーネルは、`6.12-0-55.12.1.el10_0.x86_64` で、OS バージョンは `Red Hat Enterprise Linux release 10.0 (Coughlan)` でした。
![](images/2025-05-25_16h21_06.png "バージョン確認")

## おわりに

はじめて業務で触ったバージョンが、3 だったと思うので約20年でバージョンアップが7回も経験できていると思うと年をとったな、と思いました。
これから操作していくなかで大きな違いなどがあれば記事にしたいと思います。
