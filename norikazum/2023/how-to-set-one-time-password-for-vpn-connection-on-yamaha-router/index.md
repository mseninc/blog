...
L2TP/IPsec VPN 接続は、Pre-Shared Key ( 事前共有鍵 ) (以降 PSK と評価) と ユーザー名、パスワードを利用して認証します。
この **パスワードが Google 認証システムの確認コード生成方法に準じたワンタイムパスワード** になります。

ルーターに **Lua スクリプトを配置し、定期的に実行させることでユーザー名に紐付くパスワード部分が定期的に変更される** 仕組みです。
...
1. Lua スクリプトの配置と関連設定
    - YAMAHA から提供されているLuaスクリプトをUSBメモリ経由でアップロードします
...
例えば、`pp auth username msen testpassword` と設定された1つめのユーザーは `PP1` になり、
`set PP1=msen:1a123biziktj12S9` を実行してセットアップキーを設定します。

この値が Google 認証システム ( Google Authenticator ) に入力するセットアップキーになります。
...
**注意点**
以降の手順でリンクから各Luaスクリプトを取得しますが、元ファイルの文字コードをそのままダウンロードするため、
各リンクに対して **「名前を付けてリンク先を保存」で保存** します。

ダウンロードするファイルは以下の3つです。

1. lua-script-one_time_password2.lua ( Shift-JIS / 5430バイト)
1. onetimepass.lua ( Shift-JIS / 2827バイト )
...
### Lua スクリプトを機器に転送
取得・修正した3つのLuaスクリプトをUSBメモリに転送します。

USBメモリを機器に接続します。
![USBメモリの差口](images/2023-02-04_13h49_57.png "USBメモリの差口")

正常に認識すると **ピロ** という電子音がします。

電子音がしない場合、認識されていません。
いくつかのUSBメモリを試した限り、認識しないUSBメモリのほうが多かったです。

**認識したUSBメモリは、RUF3-K32GB でした** 。
![RUF3-K32GB-BL](images/2023-02-04_13h58_37.png "RUF3-K32GB-BL")

USBメモリを認識させたあと、以下のコマンドでUSBから機器のルートにコピーします。
...
スクリプトファイルを差し替えする場合は、削除してから再度してから再度コピーします。
...
Lua スクリプトを自動的に起動するため、以下のようにパスとスケジュール設定を行います。
...
`lua-script-one_time_password2.lua` は、 `onetimepass.lua` と `sha1.lua` が連携して動作しているため、どちらかのファイルがない場合は分かるようなエラーがでますが、上記の `lua: error exit` でははっきりしとした理由が分かりません。 ※エラーを解消した経緯は後述します。
...
前述の実行確認で、Lua スクリプトのエラーですが、YAMAHAサポートの力もお借りしシューティングしましたが、1ヶ月程度解消しませんでした。

1ヶ月ほど時間がかかってしまいましたが、最終的に `set PP` の部分のキーを修正することで解決しました。
...
大文字が問題と思われましたが、YAMAHAのページには **英字の大文字小文字は区別されません** と記載されています。

![英字の大文字小文字は区別されない](images/2023-02-04_15h05_38.png "英字の大文字小文字は区別されない")

この部分は現在もYAMAHAサポートにて調査中となっています。

2要素認証を利用することで、セキュリティは格段にアップします。
昨今、VPN接続が関連した事件・事故が多いのでこの記事を参考に検討いただけましたら幸いです。
