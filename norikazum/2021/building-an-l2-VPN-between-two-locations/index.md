...
以下のYAMAHAドキュメントを参考にしています。
[L2TPv3を用いたL2VPN](http://www.rtpro.yamaha.co.jp/RT/docs/l2tpv3/#setting5)
...
**OCN(プロバイダー) から提供されている情報でインターネットに接続する設定を実施する以外、特別な設は不要** です。
...
接続後、不定IP側(構築環境) と 固定IP側(本番環境) 間で通信ができるようになっているはずです。

## あとがき

本記事ではアグレッシブモードでの接続を紹介しました。
作業中にうまく接続が確立せず、メインモードに変更すると接続できたという現象が発生しました。

あとがきになりますが、メインモードに変更したコンフィグについても紹介します。

**メインモードに変更するにあたり設定の違い** は、以下のとおりです。
...
ただし、`OG410Xa` を再起動するとグローバルIPアドレスは通常変更されますのでレスポンダー側の設定が変更になりますので注意が必要です。
