...
「[OSI 参照モデルでの Ethernet の立ち位置](#osi-参照モデルでの-ethernet-の立ち位置)」章で、Ethernet は物理層・データリンク層に対応すると記載しましたが、厳密には Ethernet の規格で対応関係が異なります。
...
### なぜ IEEE802.2 と IEEE802.3 の2つに分かれているのか

IEEE802.2 はイーサネット (IEEE802.3) 以外の物理層の違いを吸収するために作成されています。

IEEE802 委員会ではイーサネット (IEEE802.3) 以外にも、トークンバス (IEEE802.4) やトークンリング (IEEE802.5)、無線 LAN (IEEE802.11) などの物理層のプロトコルも策定されています。

データリンク層より上の階層でこれらの違いを意識しなくて良いように IEEE802.2 で LLC (Logical Link Control) 副層が定義されています。

![IEEE802.2 とその他のプロトコルの関係](images/8-relation-of-osi-and-protocol-802.png "IEEE802.2 とその他のプロトコルの関係")
...
データ領域部にどのようなタイプ (プロトコル) のデータが入っているかを示します。  
