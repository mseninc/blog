...
`name` で通知メールの件名を取得し、`timestamp` で通知日時を取得しています。  
※ timestamp は UNIX 形式なので、日付文字列に変更しています。
