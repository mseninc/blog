...
検出したテキストデータはテキストファイルにし、別の S3 にアップロードするようにします。
...
`iam.role.statements` では、 S3 と Textract の捜査権限を付与しています。
...
今後似たような構成を構築するときも今回作成した Serverless.yml を参考にできるので、作れば作るほど効率的になっていきそうです。
