...
AWS では Web アプリケーションをデプロイ、公開するサービスとして Amplify がありますが、 Ruby on Rails など Amplify では対応していないフレームワークが存在しています。

その場合、 EC2 に用意した仮想マシン上に Web アプリケーションを展開して公開する必要があります。
...
次に IP アドレスの固定化とインスタンスの外部公開を行うために Elastic IP の割り当てを行います。
...
続いて、インバウンドルールを設定して、外部からインスタンスにアクセスできるようにします。
