...
`s1` と `s2` は同じメモリーを参照しているため、二重解放が発生してしまうように見えます。
...
以下のコードでは `takes_ownership()` に渡された文字列はそのまま所有権がムーブし、関数の終了とともに解放されています。
そして、 `makes_copy()` に渡された変数はそのまま値が関数内にコピーされるため、関数が終了してもそのまま変数を利用し続けることができます。
