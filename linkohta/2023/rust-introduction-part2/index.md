...
本記事は [Part1](/rust-introduction-part1) の続きになっております。
...
関数例では `main()` の後ろで `test_function()` を宣言していますが、関数の宣言場所はどこでも問題ありません。

どこかで宣言されてさえいれば問題ありません。
...
**文はなんらかの動作をして値を返さない命令で、式は値が評価される命令を指しています。**
...
上の例では `x` に代入している `{}` が式、 `println!("The value of y is: {}", x);` が文です。
...
`return` で値を指定することもできますが、多くの関数は最後の式を暗黙的に返します。
...
**`return` で戻り値を返す場合はセミコロンがあっても式として成立します。**
...
他のプログラミング言語のように `else if` で複数の条件に応じて処理内容を変えることも `else` で条件を満たさない場合の処理を指定することもできます。
...
`if` 文の条件は論理値であればよいため、 `let bool = true` のように宣言された変数をそのまま `if bool {}` というように条件に利用することもできます。

また、 `let` 内で `if` 文を使用することもできます。
...
上記の例では `i` に 1 づつ加算していき、 9 になるとループを終了するようになっています。

また、強制的に次のループに移行させる `continue` も存在します。

ループ内にループがある場合、 **`break` と `continue` は最も内側のループに適用されます。**

**ループラベルを使用することで `break` や `continue` が適用されるループを指定することができます。**
...
最初のラベルの無い `break` は内側のループを終了させます。
...
配列のコレクションを順に取得する必要がある場合、 `for` 文を使ってループさせることができます。
