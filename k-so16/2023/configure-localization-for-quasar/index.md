...
アプリケーションを地域や国、言語の違いを問わず利用できるようにすることを **国際化** (Internationalization) といいます。たとえば、ユーザーが日本語の利用者ならアプリケーションの言語を日本語で表示し、韓国語の利用者なら韓国語で表示するように、 **そのユーザーに合わせた言語で表示する** ことが挙げられます。
...
最後に、 `src/boot/i18n/index.js` の **`createI18n()` の `locale` オプション** に、 `export` した際のキーを文字列で指定します。今回は日本語をデフォルトの表示言語として指定するために、 `locale` オプションに `ja-JP` を指定します。
